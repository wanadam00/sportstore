...
const openEditModal = (brand, index) => {
    ...
    form.brand_images = brand.brand_images;
...
const deleteImage = async (pimage, index) => {
    try {
        await router.delete('/brands/image/' + pimage.id, {
            onSuccess: (page) => {
                form.brand_images.splice(index, 1);
                Swal.fire({
                    toast: true,
                    icon: "success",
                    position: "top-end",
...
const updateBrand = async () => {
    ...
    // Append product images to the FormData
    form.brand_images.forEach((image, index) => {
        formData.append(`brand_images[${index}]`, image.raw || image);
    });
    try {
        await router.post(route('brands.update', form.id), formData, {
            onSuccess: (page) => {
                dialogVisible.value = false;
                Swal.fire({
                    toast: true,
...
const deleteBrand = (brand, index) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                form.delete('brands/destory/' + brand.id, {
                    onSuccess: (page) => {
...
<template>
    <section class="  p-3 sm:p-5">
        <div class="pb-4 mx-auto max-w-screen-xl px-4 lg:px-12 flex items-center">
            <button @click="searchBrands" class="absolute ml-2">
            ...
            <input v-model="search" placeholder="Search by name" @keyup.enter="searchBrands"
                ...
                <div class="grid  md:gap-6">
                    <div class="relative z-0 w-full mb-6 group">
                        <el-upload v-model:file-list="brandImages" list-type="picture-card" multiple
                            ...
                            <el-icon>
                                <Plus />
                                <span v-if="form.errors.brand_images" class="text-red-500 text-xs">{{
                                    form.errors.brand_images }}</span>
                            </el-icon>
                        </el-upload>
                    </div>
                </div>
